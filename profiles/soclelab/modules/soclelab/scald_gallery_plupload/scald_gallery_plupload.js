...
Drupal.behaviors.scald_gallery_pl = {
  attach: function(context, settings) {
    ...
    if ($('#modalContent').find('.field-type-atom-reference').length) {
      Drupal.behaviors.scald_gallery_pl.config.zIndex = $('.dnd-library-wrapper').css('z-index');
    ...
    // in the first place.
    $('.dnd-library-wrapper')
      .find('.meta.type-gallery-pl .edit a, .add-buttons .add-gallery-pl a')
      .removeClass('ctools-modal-custom-style')
      .addClass('ctools-modal-scald_gallery_pl');
  },
  detach: function(context, settings) {
    $('.dnd-library-wrapper').css({
      'z-index': Drupal.behaviors.scald_gallery_pl.config.zIndex
    });
  },
  config: {
    zIndex: 999
...
$(document).bind('CToolsDetachBehaviors', Drupal.behaviors.scald_gallery_pl.detach);
