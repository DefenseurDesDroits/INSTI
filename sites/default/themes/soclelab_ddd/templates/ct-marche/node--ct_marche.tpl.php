...
<article class="<?php print $classes; ?> node-vm-full clearfix" <?php print $attributes; ?>>
  <div class="row">
    ...
    <div class="col-xs-12 col-sm-6 col-md-6 col-lg-6 align-right">
      <?php if( isset($node->field_marche_type['und'][0]['tid'])){ ?>
        <div class="node-type type-marche">
          <?php $link_taxo = slutil_get_taxo_link($node->field_marche_type['und'][0]['tid']); ?>
          <a href="/marches-publics" title="<?php print $link_taxo['title']; ?>"><?php print $link_taxo['name']; ?></a>
        </div>
      <?php } ?>
  ...
  <?php if (isset($content['field_marche_limite'])){?>
    <div class="node-date date-marche text-em">
      <div class="field-label-bo"><?php print t("Date limite :"); ?></div>
      <div class="field-value"><?php print render($content["field_marche_limite"]); ?></div>  
    </div>
  <?php } ?>
    
  <?php if (isset($content["field_marche_descriptif"])) { ?>
    <div class="node-chapo chapo-marche">
      <?php print render($content["field_marche_descriptif"]); ?>
    </div>
  <?php } ?>
  

    
  <?php if (isset($content["field_marche_body"])) { ?>
    <div class="node-body body-marche text-riche">
      <?php print render($content["field_marche_body"]); ?>
    </div>
  <?php } ?>
  
  <?php if (isset($content["field_marche_url"])) { ?>
    <div class="node-url document-url">
      <?php print render($content["field_marche_url"]); ?>
    </div>
  <?php } ?>
  
  <?php if (isset($content["field_article_document"])) { ?>
    <div class="node-document document-marche">
      <?php print render($content["field_article_document"]); ?>
    </div>
  <?php } ?>
  
  
  <?php if (isset($node->field_marche_descriptif['und'][0]['value'])) { ?>
    <?php $chapo_length = strlen($node->field_marche_descriptif['und'][0]['value']); ?>
  <?php } else { $chapo_length = 0; } ?>
  <?php if (isset($node->field_marche_body['und'][0]['value'])) { ?>
    <?php $body_length = strlen($node->field_marche_body['und'][0]['value']); ?>
  ...
  <?php if ($article_length > 2500) { //TODO : rendre le paramètre configurable ?>
    <div class="keywords-marche clearfix row">
      <?php if( isset($node->field_marche_keyword['und'][0]['tid'])){ ?>
        <div class="col-xs-12 col-sm-6 col-lg-6 col-lg-6 pull-left">
          <div class="node-motcle motcle-marche">
            <span class="bloc-inline-title"><?php print t("Keywords:"); ?></span>
            <?php foreach ($node->field_marche_keyword['und'] as $key => $cur_tid) { ?>
              <?php $link_taxo = slutil_get_taxo_link($cur_tid['tid']); ?> 
              <a href="<?php print $link_taxo['url']; ?>" title="<?php print $link_taxo['title']; ?>"><?php print $link_taxo['name']; ?></a>
            <?php }?>
          </div>
      ...
      <?php } ?>
      <?php if( isset($node->field_marche_theme['und'][0]['tid'])){ ?>
        <div class="col-xs-12 col-sm-6 col-lg-6 col-lg-6 pull-right">
          <div class="node-theme theme-marche">
            <span class="bloc-inline-title"><?php print t("Institution's competence:"); ?></span>
            <?php foreach ($node->field_marche_theme['und'] as $key => $cur_tid) { ?>
              <?php $link_taxo = slutil_get_taxo_link($cur_tid['tid']); ?> 
              <a href="<?php print $link_taxo['url']; ?>" title="<?php print $link_taxo['title']; ?>"><?php print $link_taxo['name']; ?></a>
            <?php }?>
          </div>
...
<article class="<?php print $classes; ?>" <?php print $attributes; ?>>
  ...
  <?php if (isset($node->field_marche_logo['und'][0]['sid'])) { ?>
    <div class="carousel-logo logo-offre">
      <div class="big-img-slide hidden-xs visible-sm visible-md visible-lg">
        <?php print slutil_get_img($node->field_marche_logo['und'][0]['sid'], "article_vignette_home");  ?>
      </div>
      <div class="small-img-slide visible-xs hidden-sm hidden-md hidden-lg">
        <?php print slutil_get_img($node->field_marche_logo['und'][0]['sid'], "home_slider_mobile");  ?>
      </div>
    </div>
  <?php } ?>
    ...
    <div class="vignette-text">      
      <?php if( isset($node->field_marche_type['und'][0]['tid'])){ ?>
        <div class="vignette-type">
          <?php $cur_taxo = taxonomy_term_load($node->field_marche_type['und'][0]['tid']); ?>
      ...
      <?php if (isset($node->field_marche_descriptif) && isset($node->field_marche_body)) { ?>
        <div class="vignette-chapo"><?php print slutil_get_txtvignette($node->field_marche_descriptif, $node->field_marche_body, 200); ?></div>
      <?php } ?>
...
<article class="<?php print $classes; ?>" >
  <div class="bg-img">  
    <?php if (isset($node->field_marche_logo['und'][0]['sid'])) { ?>
      <?php print slutil_get_img($node->field_marche_logo['und'][0]['sid'], "taxo_vignette_home");  ?>
    <?php } elseif (isset($node->field_marche_type['und'][0]['tid'])) { ?>
      <?php $taxo = taxonomy_term_load($node->field_marche_type['und'][0]['tid']); ?>
  ...
  <div class="container">
    <div class="container-text">
      ...
      <div class="vignette-title title-marche">
        <a href="<?php print $node_url; ?>" title="<?php print $node_url_title; ?>"><?php print $title; ?></a>
      </div>
            
      <?php if (isset($node->field_marche_descriptif) && isset($node->field_marche_body)) { ?>
        <div class="vignette-chapo chapo-marche">
          <?php print slutil_get_txtvignette($node->field_marche_descriptif, $node->field_marche_body, 255); ?>
        </div>
      <?php } ?>
    </div>
  </div>
...
<article class="<?php print $classes; ?> col-xs-12 col-sm-5 col-md-4 col-lg-3 acc-taxo-node" >
  ...
  <?php if (isset($node->field_article_date['und'][0]['value'])) { ?>
    ...
    <div class="vignette-date date-marche">
...
<article class="<?php print $classes; ?> vignette-related-content <?php print $node_classes; ?> " >
  <div class="container-logo">
    ...
    <?php if (isset($node->field_marche_date['und'][0]['value'])) { ?>
      <?php $timestamp_date_article = strtotime($node->field_marche_date['und'][0]['value']); ?>
      <div class="date-marche">
        <div class="date-day"><?php print format_date($timestamp_date_article, 'base_day'); ?></div>
  ...
  <div class="title-marche">
    <a href="<?php print $node_url; ?>" title="<?php print $node_url_title; ?>"><?php print $title; ?></a>
    <div class="bottom-arrow"></div>
  </div>
  <?php if (isset($node->field_marche_descriptif) && isset($node->field_marche_body)) { ?>
    <div class="marche-chapo"><?php print slutil_get_txtvignette($node->field_marche_descriptif, $node->field_marche_body, 150, false); ?><span class="ellipsis">...</span></div>
  <?php } ?>
</article>
...
<article class="<?php print $classes; ?>" <?php print $attributes; ?>>
  <div class="row">
    <div class="col-xs-4 col-sm-5 col-md-4 col-lg-3">
      <?php if (isset($node->field_marche_logo['und'][0]['sid'])) { ?>
        <div class="sidebar-logo logo-offre">
          <a href="<?php print $node_url; ?>" title="<?php print $node_url_title; ?>">
            <?php print slutil_get_img($node->field_marche_logo['und'][0]['sid'], "sidebar_offre");  ?>
          </a>
        </div>
      <?php } ?>
    </div>
    ...
    <div class="col-xs-8 col-sm-7 col-md-8 col-lg-9">
      <div class="sidebar-title title-marche">
        <a href="<?php print $node_url; ?>" title="<?php print $node_url_title; ?>"><?php print $title; ?></a>
      </div>
      
      <?php if (isset($node->field_marche_descriptif) && isset($node->field_marche_body)) { ?>
        <div class="sidebar-chapo chapo-marche">
          <?php print slutil_get_txtvignette($node->field_marche_descriptif, $node->field_marche_body, 100); ?>
        </div>
      <?php } ?>
    </div>
  </div>
...
<article class="<?php print $classes; ?>" <?php print $attributes; ?>>
  ...
  <?php if (isset($node->field_article_logo['und'][0]['sid'])) { ?>
    <div class="carousel-logo logo-marche">
      <div class="big-img-slide hidden-xs visible-sm visible-md visible-lg">
        <?php print slutil_get_img($node->field_marche_logo['und'][0]['sid'], "article_carousel_home");  ?>
      </div>
      <div class="small-img-slide visible-xs hidden-sm hidden-md hidden-lg">
        <?php print slutil_get_img($node->field_marche_logo['und'][0]['sid'], "home_slider_mobile");  ?>
      </div>
  ...
  <div class="container">
    <div class="carousel-text">
      ...
      <div class="carousel-title carousel-marche">
        <a href="<?php print $node_url; ?>" title="<?php print $node_url_title; ?>"><?php print $title; ?></a>
      </div>
      
      <?php if (false && isset($content["field_marche_date"])) { ?>
        <div class="carousel-date date-marche">
          <?php print drupal_render($content["field_marche_date"]); ?>
        </div>
      <?php } ?>
    
      <?php if (isset($node->field_marche_descriptif) && isset($node->field_marche_body)) { ?>
        <div class="carousel-chapo chapo-marche"><?php print slutil_get_txtvignette($node->field_marche_descriptif, $node->field_marche_body, 200); ?></div>
      <?php } ?>
...
<article class="<?php print $classes; ?> search-result" <?php print $attributes; ?>>
  <div class="row">
    ...
    <div class="col-xs-12 col-sm-7 col-md-8 col-lg-8 search-content-part">
      ...
      <div class="search-title title-marche">
        <a href="<?php print $node_url; ?>" title="<?php print $node_url_title; ?>"><?php print $title; ?></a>
      </div>
      
                
      <?php if (isset($node->field_marche_descriptif) && isset($node->field_marche_body)) { ?>
        <div class="search-chapo chapo-marche text-em">
          <?php print slutil_get_txtvignette($node->field_marche_descriptif, $node->field_marche_body, 200); ?>
          <?php if (isset($node->field_marche_url['und'][0]['url'])) {?>
            <br />
            <a href="<?php print $node->field_marche_url['und'][0]['url']; ?>" title="<?php print $node->field_marche_url['und'][0]['title']; ?>">
            <?php print $node->field_marche_url['und'][0]['title']; ?>
            </a>
          <?php } ?>
      ...
      <?php if( isset($node->field_marche_type['und'][0]['tid'])){ ?>
        <div class="search-type type-marche text-em">
          <?php $list_taxo = slutil_get_list_tid($node->field_marche_type['und'][0]['tid']); ?>
          <?php $i=0; ?>
    ...
    <?php if (isset($content["field_article_document"])) { ?>
        <div class="search-logo logo-offre">
          <a href="<?php print $node_url; ?>" title="<?php print $node_url_title; ?>">
