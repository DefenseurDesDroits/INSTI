...
<?php if(isset($views_base->display['page_reglement']->display_options['path'])) { ?>
   <?php $url_views = "/" . getCurLang() . "/" . $views_base->display['page_reglement']->display_options['path']; ?>
<?php } else { $url_views = "/" . getCurLang() . "/reglement-amiable"; } ?>
...
<?php } ?>
<div class="section-dec col-xs-6 col-sm-4 col-md-2 col-lg-2 <?php if(strstr(current_path(), "reglement-amiable/date")) { print 'actif'; }?>">
  <h2>
    <a href="<?php print $url_views_date; ?>" title="<?php print slutil_encode_title(t("Display the settlement agreements page with date filter")); ?>">
       <div class="container-theme"><?php print t("Settlement agreements by date"); ?></div>
    </a>
  </h2>
</div>